  ...
  return (
    <div>
      <div className="liquid-glass rounded-2xl p-6 mb-8">
        <h1 className="text-2xl font-bold text-white mb-2">Popular Anime</h1>
      </div>
        ...
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            {items.map((item, index) => {
              const globalRank = pagination ? (currentPage - 1) * 30 + index + 1 : index + 1;
              return (
                <GlassCard
                  key={item.id}
                  href={getHref(item)}
          ...
          {pagination && pagination.total_pages > 1 && (
            <div className="flex justify-center gap-2 mt-8">
              {pagination.has_prev_page && (
                <Link
                  href={`/populer?page=${pagination.prev_page}`}
              ...
              {pagination.has_next_page && (
                <Link
                  href={`/populer?page=${pagination.next_page}`}
                  className="glass-button px-4 py-2 text-sm text-zinc-300"
                >
                  Next →
                </Link>
